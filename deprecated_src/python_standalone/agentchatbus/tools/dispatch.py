...
from agentchatbus.db.database import get_db
import sys
import importlib
if "agentchatbus.config" in sys.modules:
    importlib.reload(sys.modules["agentchatbus.config"])
if "agentchatbus.db.models" in sys.modules:
    importlib.reload(sys.modules["agentchatbus.db.models"])
if "agentchatbus.db.crud" in sys.modules:
    importlib.reload(sys.modules["agentchatbus.db.crud"])
...
from agentchatbus.config import (
    BUS_VERSION,
    HOST,
    PORT,
    MSG_WAIT_TIMEOUT,
...
async def handle_bus_connect(db, arguments: dict[str, Any]) -> list[types.TextContent]:
    ...
    agentchatbus.mcp_server._current_agent_id.set(agent.id)
    agentchatbus.mcp_server._current_agent_token.set(agent.token)
...
async def handle_bus_get_config(db, arguments: dict[str, Any]) -> list[types.TextContent]:
    session_lang = agentchatbus.mcp_server._session_language.get()
...
async def handle_msg_edit(db, arguments: dict[str, Any]) -> list[types.TextContent]:
    ...
    connection_agent_id, _ = agentchatbus.mcp_server.get_connection_agent()
...
async def handle_msg_wait(db, arguments: dict[str, Any]) -> list[types.Content]:
    ...
    connection_agent_id, connection_token = agentchatbus.mcp_server.get_connection_agent()
    ...
    #   admin coordinator loop runs in the HTTP server process (agentchatbus.main).
    ...
    async def _refresh_heartbeat() -> None:
        ...
        try:
            session_id = agentchatbus.mcp_server.get_session_id()
            if session_id:
                agentchatbus.mcp_server.mark_sse_connected(session_id)
        except Exception:
            pass
...
async def handle_agent_heartbeat(db, arguments: dict[str, Any]) -> list[types.TextContent]:
    ...
    if ok:
        agentchatbus.mcp_server._current_agent_id.set(arguments["agent_id"])
        agentchatbus.mcp_server._current_agent_token.set(arguments["token"])
        agentchatbus.mcp_server.set_connection_agent(arguments["agent_id"], arguments["token"])
