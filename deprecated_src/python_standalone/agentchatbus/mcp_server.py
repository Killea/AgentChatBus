...
from agentchatbus.config import BUS_VERSION, HOST, PORT, MSG_WAIT_TIMEOUT, EXPOSE_THREAD_RESOURCES
...
async def list_tools() -> list[types.Tool]:
    tools = [
        ...
        types.Tool(
            ...
            description=(
                ...
                "(current_seq, reply_token, reply_window). Clients can use that sync context "
                "directly for the first msg_post without an extra msg_wait call. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "system_prompt": {"type": "string",
                                      "description": "Optional system prompt for thread creation. Only applied when creating a new thread (ignored when joining an existing one). Overrides template default."},
                    "template":      {"type": "string",
                                      "description": "Optional template ID for thread creation. Only applied when creating a new thread (ignored when joining an existing one)."},
                },
    ...
    from agentchatbus.config import ENABLE_HANDOFF_TARGET, ENABLE_STOP_REASON, ENABLE_PRIORITY
    for t in tools:
        if t.name == "msg_post":
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[types.Content]:
    ...
    import importlib
    import sys
    from agentchatbus.config import RELOAD_ENABLED
    ...
    if "agentchatbus.tools.dispatch" in sys.modules:
        if RELOAD_ENABLED:
            importlib.reload(sys.modules["agentchatbus.tools.dispatch"])
    else:
        import agentchatbus.tools.dispatch

    return await sys.modules["agentchatbus.tools.dispatch"].dispatch_tool(db, name, arguments)
